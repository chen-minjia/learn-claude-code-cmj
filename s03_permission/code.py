...
# Gate 1: Hard deny list - always forbidden 硬拒绝，不能执行
DENY_LIST = ["rm -rf /", "sudo", "shutdown", "reboot", "mkfs", "dd if=", "> /dev/sda"]

# 如果 command 命中 DENY_LIST 中的命令，返回拒绝原因
...
PERMISSION_RULES = [
    {
        ...
        # 一个函数（lambda），返回`True` 就表示"命中，危险，需要拦"
            # 文件操作不能越界：防止模型读写工作区之外的文件
...
# 一条条规则去校验 tool_name 是否命中了规则中的 check 函数，命中了返回 rule["message"]，否则返回 None
